...
我们在`Application/Common/Models`中定义一个类，表示分页结果。

- `PaginatedList.cs`

```powershell
New-Item .\src\TodoList.Application\Common\Models\PaginatedList.cs
```
...
在`Application/Common/Mappings`中新增一个类用于实现关于分页结果的扩展方法：

- `MappingExtensions.cs`

```powershell
New-Item .\src\TodoList.Application\Common\Mappings\MappingExtensions.cs
```
...
为了演示分页查询的应用，我们新增一个允许分页查询`TodoItem`的`Query`：

- `GetTodoItemsWithPaginationQuery.cs`

```powershell
New-Item .\src\TodoList.Application\TodoItems\Queries\GetTodoItems\GetTodoItemWithPaginationQuery.cs -Force
```
...
public class GetTodoItemsWithPaginationQueryHandler : IRequestHandler<GetTodoItemsWithPaginationQuery, PaginatedList<TodoItemDto>>
{
    ...
    {
        return await _repository
            .GetAsQueryable(x => x.ListId == request.ListId)//报错，需要改造
            .OrderBy(x => x.Title)
...
- 请求
  ...
  ```powershell
  https://localhost:7197/todo-item?ListId=3958043A-C68E-4ED3-2D14-08DAF17FF6E3&PageNumber=2&PageSize=5
  ```
...
- 响应
  ...
  ```json
  {
  	"data": {
  		"items": [
  			{
  				"id": "189e3922-b6d4-47b6-9adb-08daf17ff6f4",
  				"listId": "3958043a-c68e-4ed3-2d14-08daf17ff6e3",
  				"title": "Toilet paper",
  				"done": false,
  				"priority": 0
  			...
  			{
  				"id": "6413d6ce-66b6-466c-9ade-08daf17ff6f4",
  				"listId": "3958043a-c68e-4ed3-2d14-08daf17ff6e3",
  				"title": "Tuna",
  				"done": false,
  				"priority": 0
  			...
  			{
  				"id": "71c88202-0637-4d38-9adf-08daf17ff6f4",
  				"listId": "3958043a-c68e-4ed3-2d14-08daf17ff6e3",
  				"title": "Water",
  				"done": false,
  				"priority": 0
