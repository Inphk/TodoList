...
3. 对`appsettings.Development.json`文件进行配置：
   ...
   begin
       ...
       (
           ...
           filename='E:\MyLearn\c#\TodoList\TodoListDB\TodoListDB_data.mdf',    -- 主数据文件的物理名称
           size=8mb,    --主数据文件的初始大小
       ...
       (
           ...
           filename='E:\MyLearn\c#\TodoList\TodoListDB\TodoListDB_log.ldf',    -- 日志文件的物理名称
           size=8mb,            --日志文件的初始大小
...
5. Docker环境正常时，运行如下命令：
   ...
   # 运行mssql容器，挂载持久化存储卷
   $ docker run -d --volumes-from mssqldata --name mssql -e 'ACCEPT_EULA=Y' -e 'SA_PASSWORD=!@#123qwe' -p 1433:1433 mcr.microsoft.com/mssql/server:2019-latest
...
```powershell
New-Item .\src\TodoList.Application\TodoItem\Commands\CreatTodoItem\CreateTodoItemCommand.cs -Force
```
